                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXG ought to be enough for anybody." - Bill Gates, 2017
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzXG->setToolTip(tr("zXG is currently disabled due to maintenance."));

        ui->pushButtonSpendzXG->setEnabled(false);
        ui->pushButtonSpendzXG->setToolTip(tr("zXG is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonMintzXG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXG in ") +
...
void PrivacyDialog::on_pushButtonSpendzXG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zXG
        sendzXG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXG
    sendzXG();
}

void PrivacyDialog::on_pushButtonZGigaControl_clicked()
{
    ZGigaControlDialog* zGigaControl = new ZGigaControlDialog(this);
    zGigaControl->setModel(walletModel);
    zGigaControl->exec();
}

void PrivacyDialog::setZGigaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGigaSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Giga Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zXG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XG </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXG</b>";
    ...
    // use mints from zGiga selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGigaControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGigaControlDialog::GetSelectedMints();
    }
    ...
    // Clear zGiga selector in case it was used
    ZGigaControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGiga Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Giga, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGiga Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXG </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXG "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXG "));
