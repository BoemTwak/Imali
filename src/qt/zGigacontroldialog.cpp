...
#include "ui_zGigacontroldialog.h"
...
std::list<std::string> ZGigaControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGigaControlDialog::listMints;

ZGigaControlDialog::ZGigaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGigaControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGigaControlDialog::~ZGigaControlDialog()
{
    delete ui;
}

void ZGigaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGigaControlDialog::updateList()
{
...
void ZGigaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGigaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGiga_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGigaControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGigaControlDialog::GetSelectedMints()
{
...
void ZGigaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
